...
import {
  ...
  Home, TrendingUp, AlertTriangle, Wifi, WifiOff, CalendarClock, Image as ImageIcon, Check, Circle, PieChart as PieIcon, ArrowLeftRight, Landmark,
} from "lucide-react";
...
/* ── cuentas ──
   Cada cuenta tiene un tipo, que es lo que antes eran los tres baldes fijos
   (pesos, plazo fijo, dólares). El tipo define cómo se muestra; la moneda,
   cómo se valúa. "inmediata" marca si podés echar mano de esa plata hoy mismo. */
const TIPOS_CUENTA = {
  caja:       { label: "Caja de ahorro",    inmediata: true },
  corriente:  { label: "Cuenta corriente",  inmediata: true },
  remunerada: { label: "Cuenta remunerada", inmediata: true },
  billetera:  { label: "Billetera virtual", inmediata: true },
  plazo:      { label: "Plazo fijo",        inmediata: false },
  efectivo:   { label: "Efectivo",          inmediata: true },
};
/* Las tres cuentas que replican exactamente el modelo viejo, para que nadie
   pierda nada al actualizar. Los ids son fijos: la migración tiene que ser
   siempre la misma, corra las veces que corra. */
const CUENTAS_BASE = [
  { id: "c-pesos", nombre: "Cuenta en pesos", tipo: "caja",  moneda: "ARS", alias: "", cbu: "" },
  { id: "c-plazo", nombre: "Plazo fijo",      tipo: "plazo", moneda: "ARS", alias: "", cbu: "" },
  { id: "c-usd",   nombre: "Dólares",         tipo: "caja",  moneda: "USD", alias: "", cbu: "" },
];
...
function migrar(x) {
  ...
  if (typeof y.guiaCerrada !== "boolean") y.guiaCerrada = false;

  /* ── cuentas ──
     Antes el saldo del mes era {pesos, plazo, dolares}. Ahora es un monto por
     cuenta. Convierto cada mes declarado a las tres cuentas base, así el que ya
     venía usando la app ve exactamente los mismos números que ayer. */
  if (!Array.isArray(y.cuentas) || !y.cuentas.length) y.cuentas = CUENTAS_BASE.map((c) => ({ ...c }));
  y.cuentas = y.cuentas.map((c) => ({
    id: c.id || uid(),
    nombre: c.nombre || "Cuenta",
    tipo: TIPOS_CUENTA[c.tipo] ? c.tipo : "caja",
    moneda: c.moneda === "USD" ? "USD" : "ARS",
    alias: c.alias || "",
    cbu: c.cbu || "",
  }));
  if (!Array.isArray(y.transferencias)) y.transferencias = [];

  const saldosMigrados = {};
  Object.entries(y.saldos).forEach(([mes, s]) => {
    if (!s || typeof s !== "object") return;
    if (s.cuentas && typeof s.cuentas === "object") { saldosMigrados[mes] = s; return; } // ya migrado
    const porCuenta = {};
    if (+s.pesos) porCuenta["c-pesos"] = +s.pesos;
    if (+s.plazo) porCuenta["c-plazo"] = +s.plazo;
    if (+s.dolares) porCuenta["c-usd"] = +s.dolares;
    saldosMigrados[mes] = { cuentas: porCuenta };
  });
  y.saldos = saldosMigrados;
  return y;
}
...
export default function App() {
  ...
  /* Un saldo del mes es un monto por cuenta. Cada cuenta se valúa en su moneda:
     las de dólares pasan por el MEP. Acepta el formato viejo por si quedó algo sin migrar. */
  const valorSaldo = useCallback((s) => {
    if (!s) return 0;
    if (s.cuentas && typeof s.cuentas === "object") {
      return Object.entries(s.cuentas).reduce((acc, [id, monto]) => {
        const c = d.cuentas.find((x) => x.id === id);
        const m = +monto || 0;
        return acc + (c?.moneda === "USD" ? m * d.mercado.mep : m);
      }, 0);
    }
    return (s.pesos || 0) + (s.plazo || 0) + (s.dolares || 0) * d.mercado.mep;
  }, [d.mercado.mep, d.cuentas]);
  ...
  const registro = useMemo(() => {
    ...
    if (!claves.size) return [];   // ojo: un Set usa .size, no .length
  ...
  /* ── estado de cada cuenta ──
     Lo declarado es la verdad; lo estimado es lo declarado más lo que anotaste
     después. Si no anotaste todo, el estimado queda corto: por eso, cuando
     declarás de nuevo, el arqueo te muestra la diferencia en vez de esconderla. */
  const cuentasEstado = useMemo(() => {
    const meses = Object.keys(d.saldos).sort();
    return d.cuentas.map((c) => {
      let ultimoMes = null, declarado = 0;
      meses.forEach((m) => {
        const v = d.saldos[m]?.cuentas?.[c.id];
        if (v !== undefined && v !== null && v !== "") { ultimoMes = m; declarado = +v || 0; }
      });
      const post = (f) => !ultimoMes || (f || "").slice(0, 7) > ultimoMes;
      const entradas = d.entradas.filter((x) => x.cuenta === c.id && post(x.fecha)).reduce((s, x) => s + x.monto, 0);
      const gastos = d.gastos.filter((x) => x.cuenta === c.id && post(x.fecha)).reduce((s, x) => s + x.monto, 0);
      const recibido = d.transferencias.filter((t) => t.hacia === c.id && post(t.fecha)).reduce((s, t) => s + (+t.montoDestino || +t.monto || 0), 0);
      const enviado = d.transferencias.filter((t) => t.desde === c.id && post(t.fecha)).reduce((s, t) => s + (+t.monto || 0), 0);
      const movimiento = entradas - gastos + recibido - enviado;
      return { ...c, ultimoMes, declarado, movimiento, estimado: declarado + movimiento, sinDeclarar: !ultimoMes };
    });
  }, [d.cuentas, d.saldos, d.entradas, d.gastos, d.transferencias]);
  ...
  const guia = useMemo(() => {
    ...
    /* Sin ingreso declarado no puedo afirmar que gastás menos de lo que entra:
       cero menos cero da cero, y eso no es un logro, es falta de datos. */
    P.push({
      id: "rojo", que: "Gastá menos de lo que entra", ir: "movimientos",
      hecho: ingreso.valor > 0 && ahorro.valor >= 0,
      por: ingreso.valor <= 0
        ? "Todavía no sé cuánto entra por mes. Cargá tu sueldo o lo que cobrás y te digo si te queda un resto."
        : ahorro.valor >= 0
    ...
    /* Ojo: la concentración en una sola plataforma es una advertencia permanente,
       no una tarea que se termina. Si la usara como condición, el que tiene todo
       en un solo lado (lo más común) nunca podría completar la guía. Va como aviso
       dentro de la sección de Inversiones, no como paso bloqueante. */
    const concentracionRiesgo = concentracion && concentracion.p > 60 && concentracion.tipo === "exterior";
    const invOk = d.inversiones.length === 0 ? colchonListo : sinPrecio.length === 0;
    P.push({
      id: "inversiones",
      ...
      por: sinPrecio.length
        ...
        ? concentracionRiesgo
          ? `Tus inversiones están al día. Un dato para tener en cuenta: el ${fmt(concentracion.p, 0)}% está en ${concentracion.name}, una sola plataforma del exterior.`
  ...
  }, [deudaCreciendo, interesesMes, totalDeuda, valorBase, ahorro, ingreso, sinPrecio, cerrados.length, mesesCubiertos, colchonListo, d.colchonMeses, concentracion, d.metas, d.inversiones, totalLiquido]);

  /* Si la guía estaba cerrada y se te destapa un paso, vuelve a aparecer sola. */
  useEffect(() => {
    if (!guia.completo && d.guiaCerrada) guardar({ ...d, guiaCerrada: false });
  }, [guia.completo, d.guiaCerrada]);
  ...
  return (
    <div className="app">
      ...
      <header className="barra">
        <span className="logo">Mi plata <span className="sello-version">v8.4 · cuentas</span></span>
      ...
      {/* La guía se esconde cuando ya no tiene nada para decirte. Si más adelante
          se te destapa algún paso (te endeudás, el colchón baja), vuelve sola. */}
      {!viendoPasado && !(guia.completo && d.guiaCerrada) && (
        <div className={`paso ${guia.completo ? "paso-completo" : ""}`}>
          {guia.completo ? (
            <div className="paso-cabeza paso-cabeza-fin">
              <span className="paso-eyebrow">Terminaste la guía</span>
              <span className="paso-que">Ya diste todos los pasos</span>
              <span className="paso-por">
                Registraste tus meses, gastás menos de lo que entra, no tenés deudas caras, armaste
                el colchón y le pusiste nombre a una meta. De acá en adelante es sostenerlo.
              </span>
            </div>
          ) : (
            <button className="paso-cabeza" onClick={() => setTab(guia.actual.ir)}>
              <span className="paso-eyebrow">Tu próximo paso · {guia.indice + 1} de {guia.total}</span>
              <span className="paso-que">{guia.actual.que}</span>
              <span className="paso-por">{guia.actual.por}</span>
          ...
          {guia.completo && (
            <div className="paso-cerrar">
              <button className="btn-secundario" onClick={() => guardar({ ...d, guiaCerrada: true })}>
                <Check size={15} /> Listo, ocultar la guía
              </button>
            </div>
          )}
        </div>
      ...
      <nav className="tabs" ref={navRef}>
        {[["hoy","Resumen",Home],["movimientos","Entra y sale",ArrowRight],["reparto","Reparto",PieIcon],["cuentas","Cuentas",Landmark],["colchon","Colchón",ShieldCheck],
      ...
      <main>
        ...
        {tab === "registro" && <Registro {...CTX} irA={setTab} registro={registro} valorSaldo={valorSaldo}
        ...
        {tab === "cuentas" && <Cuentas {...CTX} cuentasEstado={cuentasEstado} />}
...
function Cuentas({ d, guardar, conv, signo, cuentasEstado, setAviso, ym }) {
  const [nombre, setNombre] = useState(""), [tipo, setTipo] = useState("caja"), [moneda, setMoneda] = useState("ARS");
  const [alias, setAlias] = useState(""), [cbu, setCbu] = useState("");
  const [editando, setEditando] = useState(null);
  const [verDatos, setVerDatos] = useState(null);
  const [tDesde, setTDesde] = useState(""), [tHacia, setTHacia] = useState("");
  const [tMonto, setTMonto] = useState(""), [tLlega, setTLlega] = useState(""), [tFecha, setTFecha] = useState(hoyISO()), [tNota, setTNota] = useState("");

  const cta = (id) => d.cuentas.find((c) => c.id === id);
  const cDesde = cta(tDesde), cHacia = cta(tHacia);
  const distintaMoneda = cDesde && cHacia && cDesde.moneda !== cHacia.moneda;

  const agregar = () => {
    if (!nombre.trim()) { setAviso && setAviso("Ponele un nombre a la cuenta."); return; }
    const nueva = { id: uid(), nombre: nombre.trim(), tipo, moneda, alias: alias.trim(), cbu: cbu.replace(/\s/g, "") };
    guardar({ ...d, cuentas: [...d.cuentas, nueva] });
    setNombre(""); setAlias(""); setCbu("");
    setAviso && setAviso(`Cuenta "${nueva.nombre}" agregada.`);
  };

  const editar = (id, campo, valor) =>
    guardar({ ...d, cuentas: d.cuentas.map((c) => (c.id === id ? { ...c, [campo]: valor } : c)) });

  const borrar = (c) => {
    const usada = d.entradas.some((x) => x.cuenta === c.id) || d.gastos.some((x) => x.cuenta === c.id)
      || d.transferencias.some((t) => t.desde === c.id || t.hacia === c.id)
      || Object.values(d.saldos).some((s) => s?.cuentas?.[c.id] !== undefined);
    if (usada && !window.confirm(`"${c.nombre}" tiene movimientos o saldos anotados. Si la borrás, esos movimientos quedan sin cuenta asignada. ¿Seguís?`)) return;
    const saldos = {};
    Object.entries(d.saldos).forEach(([m, s]) => {
      const cs = { ...(s.cuentas || {}) }; delete cs[c.id];
      saldos[m] = { ...s, cuentas: cs };
    });
    guardar({
      ...d, saldos,
      cuentas: d.cuentas.filter((x) => x.id !== c.id),
      entradas: d.entradas.map((x) => (x.cuenta === c.id ? { ...x, cuenta: undefined } : x)),
      gastos: d.gastos.map((x) => (x.cuenta === c.id ? { ...x, cuenta: undefined } : x)),
      transferencias: d.transferencias.filter((t) => t.desde !== c.id && t.hacia !== c.id),
    });
    setAviso && setAviso(`Borré la cuenta "${c.nombre}".`);
  };

  const transferir = () => {
    const m = +tMonto || 0;
    const llega = distintaMoneda ? (+tLlega || 0) : m;
    if (!tDesde || !tHacia) { setAviso && setAviso("Elegí de qué cuenta sale y a cuál entra."); return; }
    if (tDesde === tHacia) { setAviso && setAviso("No podés pasar plata a la misma cuenta."); return; }
    if (m <= 0) { setAviso && setAviso("Poné cuánto pasás."); return; }
    if (distintaMoneda && llega <= 0) { setAviso && setAviso("Poné cuánto llega a la otra cuenta."); return; }
    guardar({ ...d, transferencias: [{ id: uid(), fecha: tFecha, desde: tDesde, hacia: tHacia, monto: m, montoDestino: llega, nota: tNota.trim() }, ...d.transferencias] });
    setTMonto(""); setTLlega(""); setTNota("");
    setAviso && setAviso(`Pasaste ${cDesde.moneda === "USD" ? "US$" : "$"}${fmt(m)} de ${cDesde.nombre} a ${cHacia.nombre}.`);
  };

  const borrarTransf = (id) => guardar({ ...d, transferencias: d.transferencias.filter((t) => t.id !== id) });

  const totalARS = cuentasEstado.reduce((s, c) => s + (c.moneda === "USD" ? c.estimado * d.mercado.mep : c.estimado), 0);
  const sinDeclarar = cuentasEstado.filter((c) => c.sinDeclarar).length;
  const mask = (v) => (v && v.length > 4 ? "•".repeat(Math.max(0, v.length - 4)) + v.slice(-4) : v);

  return (
    <div className="grid">
      <div className="tarjeta ancho">
        <span className="stat-l">Lo que hay en tus cuentas</span>
        <div className="colchon-num">{signo}{fmt(conv(totalARS))}</div>
        <p className="colchon-texto">
          Es la suma de lo último que declaraste en cada cuenta, más lo que anotaste después.
          {sinDeclarar > 0 && <> Te {sinDeclarar === 1 ? "falta declarar 1 cuenta" : `faltan declarar ${sinDeclarar} cuentas`}: hasta que lo hagas, esa cuenta cuenta como cero.</>}
        </p>
      </div>

      <div className="tarjeta ancho">
        <h3>Tus cuentas <span className="conteo">{d.cuentas.length}</span></h3>
        {!d.cuentas.length ? <p className="chico">No tenés ninguna cuenta cargada. Agregá la primera abajo.</p> : (
          <ul className="ctas">
            {cuentasEstado.map((c) => {
              const mon = c.moneda === "USD" ? "US$" : "$";
              const abierta = editando === c.id;
              return (
                <li key={c.id} className="cta">
                  <div className="cta-fila">
                    <div className="cta-izq">
                      <div className="cta-top">
                        <span className="cta-nombre">{c.nombre}</span>
                        <span className="chip chip-neutro">{TIPOS_CUENTA[c.tipo]?.label || c.tipo}</span>
                        {c.moneda === "USD" && <span className="chip chip-ok">dólares</span>}
                      </div>
                      {(c.alias || c.cbu) && (
                        <div className="cta-datos">
                          {c.alias && <span className="cta-dato"><b>alias</b> {verDatos === c.id ? c.alias : mask(c.alias)}</span>}
                          {c.cbu && <span className="cta-dato"><b>CBU</b> {verDatos === c.id ? c.cbu : mask(c.cbu)}</span>}
                          <button className="btn-fantasma" onClick={() => setVerDatos(verDatos === c.id ? null : c.id)}>
                            {verDatos === c.id ? "ocultar" : "ver"}
                          </button>
                          {verDatos === c.id && (
                            <button className="btn-fantasma" onClick={() => {
                              const txt = [c.alias && `alias: ${c.alias}`, c.cbu && `CBU: ${c.cbu}`].filter(Boolean).join(" · ");
                              navigator.clipboard?.writeText(txt);
                              setAviso && setAviso("Copiado.");
                            }}>copiar</button>
                          )}
                        </div>
                      )}
                      <span className="li-fecha">
                        {c.sinDeclarar
                          ? "todavía no declaraste el saldo de esta cuenta"
                          : <>declarado en {nombreMes(c.ultimoMes)}{c.movimiento !== 0 && <> · {c.movimiento > 0 ? "+" : "−"}{mon}{fmt(Math.abs(c.movimiento))} anotado después</>}</>}
                      </span>
                    </div>
                    <div className="cta-der">
                      <span className="monto">{mon}{fmt(c.estimado)}</span>
                      {c.moneda === "USD" && <span className="li-fecha">≈ ${fmt(c.estimado * d.mercado.mep)}</span>}
                      <div className="pos-acciones">
                        <button className="btn-fantasma" onClick={() => setEditando(abierta ? null : c.id)}>{abierta ? "listo" : "editar"}</button>
                        <button className="btn-x" onClick={() => borrar(c)} title="Borrar cuenta"><Trash2 size={13} /></button>
                      </div>
                    </div>
                  </div>
                  ...
                  {abierta && (
                    <div className="cta-edit">
                      <div className="campos-fila">
                        <label>Nombre<input value={c.nombre} onChange={(e) => editar(c.id, "nombre", e.target.value)} /></label>
                        <label>Qué tipo de cuenta es
                          <select value={c.tipo} onChange={(e) => editar(c.id, "tipo", e.target.value)}>
                            {Object.entries(TIPOS_CUENTA).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                          </select>
                        </label>
                        <label>Moneda
                          <select value={c.moneda} onChange={(e) => editar(c.id, "moneda", e.target.value)}>
                            <option value="ARS">Pesos</option><option value="USD">Dólares</option>
                          </select>
                        </label>
                        <label>Alias<input value={c.alias} onChange={(e) => editar(c.id, "alias", e.target.value)} placeholder="mi.alias.banco" /></label>
                        <label>CBU<input value={c.cbu} onChange={(e) => editar(c.id, "cbu", e.target.value.replace(/\s/g, ""))} placeholder="22 números" inputMode="numeric" /></label>
                      </div>
                    </div>
                  )}
                </li>
      ...
      <div className="tarjeta">
        <h3>Agregar una cuenta</h3>
        <div className="campos">
          <label>Cómo le decís<input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Galicia, Mercado Pago, Efectivo…" /></label>
          <label>Qué tipo es
          ...
          <label>Alias (opcional)<input value={alias} onChange={(e) => setAlias(e.target.value)} placeholder="mi.alias.banco" /></label>
          <label>CBU (opcional)<input value={cbu} onChange={(e) => setCbu(e.target.value.replace(/\s/g, ""))} placeholder="22 números" inputMode="numeric" /></label>
        </div>
        <button className="btn-primario ancho-btn" onClick={agregar}><Plus size={15} /> Agregar cuenta</button>
        <p className="pie">
          El alias y el CBU son solo para tenerlos a mano: la app no los usa para nada. Se guardan en tu
          navegador, igual que el resto. Por eso los muestro tapados y hay que tocar "ver" para leerlos.
        </p>
      </div>

      <div className="tarjeta">
        <h3>Pasar plata de una cuenta a otra</h3>
        {d.cuentas.length < 2 ? <p className="chico">Necesitás al menos dos cuentas para mover plata entre ellas.</p> : (
          <>
            <div className="campos">
              <label>De dónde sale
                <select value={tDesde} onChange={(e) => setTDesde(e.target.value)}>
                  <option value="">Elegí una cuenta</option>
                  {d.cuentas.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                </select>
              </label>
              <label>A dónde entra
                <select value={tHacia} onChange={(e) => setTHacia(e.target.value)}>
                  <option value="">Elegí una cuenta</option>
                  {d.cuentas.filter((c) => c.id !== tDesde).map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                </select>
              </label>
              <label>Cuánto sale{cDesde ? ` (${cDesde.moneda === "USD" ? "US$" : "$"})` : ""}
                <input type="number" min="0" step="any" value={tMonto} onChange={(e) => setTMonto(e.target.value)} placeholder="0" />
              </label>
              {distintaMoneda && (
                <label>Cuánto llega ({cHacia.moneda === "USD" ? "US$" : "$"})
                  <input type="number" min="0" step="any" value={tLlega} onChange={(e) => setTLlega(e.target.value)} placeholder="0" />
                </label>
              )}
              <label>Cuándo<input type="date" max={hoyISO()} value={tFecha} onChange={(e) => setTFecha(e.target.value)} /></label>
              <label>Nota (opcional)<input value={tNota} onChange={(e) => setTNota(e.target.value)} placeholder="compré dólares, pagué la tarjeta…" /></label>
            </div>
            {distintaMoneda && tMonto > 0 && tLlega > 0 && (
              <p className="reparto-aviso">
                Te queda a un cambio de ${fmt((+tMonto) / (+tLlega) > 1 ? (+tMonto) / (+tLlega) : (+tLlega) / (+tMonto))} por dólar.
              </p>
            )}
            <button className="btn-primario ancho-btn" onClick={transferir}><ArrowLeftRight size={15} /> Anotar el movimiento</button>
            <p className="pie">
              Pasar plata de una cuenta a otra <b>no es un gasto ni un ingreso</b>: tu patrimonio no cambia,
              solo cambia dónde está. Por eso no aparece en "entra y sale".
            </p>
          </>
        )}
      </div>
      ...
      <div className="tarjeta ancho">
        <h3>Movimientos entre cuentas <span className="conteo">{d.transferencias.length}</span></h3>
        {!d.transferencias.length ? <p className="chico">Todavía no anotaste ningún movimiento entre cuentas.</p> : (
          <ul className="ops-lista">
            {d.transferencias.slice().sort((a, b) => (b.fecha || "").localeCompare(a.fecha || "")).map((t) => {
              const cd = cta(t.desde), ch = cta(t.hacia);
              const md = cd?.moneda === "USD" ? "US$" : "$";
              const mh = ch?.moneda === "USD" ? "US$" : "$";
              const cambio = cd && ch && cd.moneda !== ch.moneda;
              return (
                <li key={t.id} className="op">
                  <span className="op-tipo transf">pase</span>
                  <div className="op-datos">
                    <span className="op-linea1">{cd?.nombre || "(cuenta borrada)"} → {ch?.nombre || "(cuenta borrada)"}</span>
                    <span className="op-linea2">{t.fecha}{t.nota ? ` · ${t.nota}` : ""}</span>
                  </div>
                  <div className="op-der">
                    <span className="op-total">{md}{fmt(t.monto)}</span>
                    {cambio && <span className="op-res">llegan {mh}{fmt(t.montoDestino ?? t.monto)}</span>}
                  </div>
                  <button className="btn-x" onClick={() => borrarTransf(t.id)} title="Borrar"><Trash2 size={13} /></button>
                </li>
              );
            })}
          </ul>
    ...
    <div className="grid">
      <div className="tarjeta">
        ...
        <div className="campos">
          ...
          {d.cuentas.length > 0 && (
            <label>{tipo === "gastos" ? "De qué cuenta salió" : "A qué cuenta entró"}
              <select value={cuenta} onChange={(e) => setCuenta(e.target.value)}>
                <option value="">No sé / no la anoto</option>
                {d.cuentas.map((c) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
              </select>
            </label>
          )}
        ...
        <button className="btn-primario ancho-btn" onClick={() => {
          ...
          add(tipo, { monto: m, fecha, categoria: cat, nota, cuenta: cuenta || undefined }); setMonto(""); setNota("");
              ...
              return (
                <li key={m.id}>
                  <div className="li-izq">
                    ...
                    <span className="li-fecha">{m.fecha}{m.cuenta && d.cuentas.find((c) => c.id === m.cuenta) ? ` · ${d.cuentas.find((c) => c.id === m.cuenta).nombre}` : ""}</span>
                  </div>
...
function Registro({ d, guardar, conv, signo, registro, valorSaldo, totalLiquido, ultimoMesSaldo, ym, setAviso, irA }) {
  const [mes, setMes] = useState(ym);
  /* un valor por cuenta, como texto para que el campo se pueda dejar vacío */
  const leer = (m) => {
    const cs = d.saldos[m]?.cuentas || {};
    const out = {};
    d.cuentas.forEach((c) => { out[c.id] = cs[c.id] !== undefined && cs[c.id] !== null ? String(cs[c.id]) : ""; });
    return out;
  };
  const [vals, setVals] = useState(() => leer(ym));

  const cambiarMes = (nuevo) => { setMes(nuevo); setVals(leer(nuevo)); };
  const setVal = (id, v) => setVals((x) => ({ ...x, [id]: v }));

  const guardarSaldo = () => {
    const porCuenta = {};
    d.cuentas.forEach((c) => { const v = +vals[c.id]; if (vals[c.id] !== "" && !Number.isNaN(v)) porCuenta[c.id] = v; });
    ...
    const nuevos = { ...d.saldos, [mes]: { cuentas: porCuenta } };
    guardar({ ...d, saldos: nuevos });
    setAviso && setAviso(`Guardado: en ${nombreMes(mes)} tenías $${fmt(valorSaldo({ cuentas: porCuenta }))}.`);
  };
  ...
  return (
    <div className="grid">
      ...
      <div className="tarjeta">
        ...
        {!d.cuentas.length ? (
          <p className="chico">
            No tenés cuentas cargadas. <button className="btn-fantasma" onClick={() => irA && irA("cuentas")}>agregá la primera</button> y
            después volvé acá a declarar cuánto tenías en cada una.
          </p>
        ) : (
          <>
            <ul className="saldo-ctas">
              {d.cuentas.map((c) => (
                <li key={c.id}>
                  <div className="sc-datos">
                    <span className="sc-nombre">{c.nombre}</span>
                    <span className="sc-tipo">{TIPOS_CUENTA[c.tipo]?.label || c.tipo}{c.moneda === "USD" ? " · dólares" : ""}</span>
                  </div>
                  <div className="sc-input">
                    <span className="sc-mon">{c.moneda === "USD" ? "US$" : "$"}</span>
                    <input type="number" min="0" step="any" value={vals[c.id] ?? ""} onChange={(e) => setVal(c.id, e.target.value)} placeholder="0" />
                  </div>
                </li>
              ))}
            </ul>
        ...
        <p className="pie">
          Anotá lo que <b>realmente</b> tenías en cada cuenta al terminar el mes: abrí el homebanking y copiá el número.
          Lo que dejes vacío no se guarda. Podés cargar meses viejos o corregir cualquiera. Solo cuenta la plata
          líquida; las inversiones van aparte.
        </p>
      </div>
...
.paso-cabeza-fin{cursor:default}
.paso-cerrar{padding:0 16px 16px;display:flex}
.paso-cerrar .btn-secundario{width:100%;color:var(--papel);border-color:rgba(231,234,227,.35)}
.paso-cerrar .btn-secundario:hover{background:rgba(231,234,227,.1);border-color:var(--papel)}
...
/* ── cuentas ── */
.ctas{list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:2px}
.cta{padding:13px 0;border-bottom:1px solid var(--linea)}
.cta:last-child{border-bottom:none}
.cta-fila{display:flex;justify-content:space-between;align-items:flex-start;gap:14px}
.cta-izq{display:flex;flex-direction:column;gap:5px;min-width:0;flex:1}
.cta-top{display:flex;align-items:center;gap:7px;flex-wrap:wrap}
.cta-nombre{font-size:15px;font-weight:500}
.cta-datos{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.cta-dato{font-family:'IBM Plex Mono',monospace;font-size:11.5px;color:var(--niebla);letter-spacing:.02em}
.cta-dato b{color:var(--tinta);font-weight:500;margin-right:4px;text-transform:uppercase;font-size:9.5px;letter-spacing:.08em}
.cta-der{display:flex;flex-direction:column;align-items:flex-end;gap:4px;flex-shrink:0}
.cta-edit{margin-top:12px;padding:14px;background:var(--papel);border:1px solid var(--linea);border-radius:8px}
.op-tipo.transf{background:#DCE3EA;color:#3D5A6C}
.saldo-ctas{list-style:none;padding:0;margin:0 0 4px;display:flex;flex-direction:column;gap:2px}
.saldo-ctas li{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 0;border-bottom:1px solid var(--linea)}
.saldo-ctas li:last-child{border-bottom:none}
.sc-datos{display:flex;flex-direction:column;gap:1px;min-width:0;flex:1;overflow:hidden}
.sc-nombre,.sc-tipo{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sc-nombre{font-size:14px;font-weight:500}
.sc-tipo{font-size:10.5px;color:var(--niebla);font-family:'IBM Plex Mono',monospace}
.sc-input{display:flex;align-items:center;gap:5px;flex-shrink:0}
.sc-mon{font-family:'IBM Plex Mono',monospace;font-size:12.5px;color:var(--niebla)}
.sc-input input{width:130px;text-align:right}
@media(max-width:720px){.sc-input input{width:120px;font-size:16px}}
@media(max-width:720px){
  .cta-fila{flex-direction:column;gap:9px}
  .cta-der{align-items:flex-start;width:100%;flex-direction:row;justify-content:space-between;align-items:center}
}
...
.li-nota{font-size:13.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;max-width:100%}
