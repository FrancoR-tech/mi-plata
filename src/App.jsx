...
export default function App() {
  ...
  /* ── posiciones: lotes agrupados, promedio ponderado ── */
  const posiciones = useMemo(() => {
    const g = {};
    d.inversiones.forEach((l) => {
      const k = clave(l.tipo, l.ticker);
      if (!g[k]) g[k] = { k, tipo: l.tipo, ticker: l.ticker, lotes: [], cantidad: 0, plataformas: new Set() };
      g[k].lotes.push(l);
      g[k].cantidad += l.cantidad;
      g[k].plataformas.add(l.plataforma);
    });
    ...
    return Object.values(g).map((p) => {
      ...
      const promedio = p.lotes.reduce((s, l) => s + l.cantidad * l.precioCompra, 0) / (p.cantidad || 1);
      ...
      const valor = precio ? p.cantidad * precio.valor * (enUSD ? dHoy : 1) : null;
      const rend = valor && costoNominal ? {
        nominal: (valor / costoNominal - 1) * 100,
        real: (valor / costoReal - 1) * 100,
        usd: ((valor / dHoy) / costoUSD - 1) * 100,
      } : null;

      return { ...p, inst, enUSD, promedio, precio, valor, rend, plataformas: [...p.plataformas] };
  ...
  const sinPrecio = posiciones.filter((p) => !p.precio);
  const preciosViejos = posiciones.filter((p) => p.precio && !p.inst.fijo && diasDesde(p.precio.fecha) > VIEJO_DIAS);

  /* ── inversiones tal como estaban en el mes que se mira ──
     en el presente: valor de mercado. en el pasado: solo lo comprado hasta ese mes, a costo. */
  const posicionesMes = useMemo(() => {
    ...
    return posiciones.map((p) => {
      ...
      const costoARS = lotes.reduce((s, l) => s + l.cantidad * l.precioCompra * (p.enUSD ? (l.dolarCompra || dolarDe(p.tipo)) : 1), 0);
      return { ...p, lotes, cantidad, valorMes: costoARS, aCosto: true };
  ...
  const porPlataforma = useMemo(() => {
    ...
    d.inversiones.forEach((l) => {
      ...
      if (!p || !p.valor) return;
      m[l.plataforma] = (m[l.plataforma] || 0) + p.valor * (l.cantidad / p.cantidad);
    });
  ...
  const CTX = { d, add, del, guardar, conv, signo, lente, ym, mesVista, viendoPasado };
    ...
    <div className="app">
      ...
      <nav className="tabs">
        {[["hoy","Resumen",Home],["movimientos","Entra y sale",ArrowRight],["colchon","Colchón",ShieldCheck],
          ...
          <button key={k} className={`tab ${tab === k ? "on" : ""}`} onClick={() => setTab(k)}><I size={14} /> {l}</button>
        ))}
      </nav>
      ...
      <main>
        ...
        {tab === "inversiones" && <Inversiones {...CTX} posiciones={posiciones} posicionesMes={posicionesMes} setPrecio={setPrecio} porPlataforma={porPlataforma}
          total={totalInvertido} invertidoMes={invertidoMes} concentracion={concentracion} dolarDe={dolarDe} sinPrecio={sinPrecio} preciosViejos={preciosViejos} />}
...
function Inversiones({ d, add, del, conv, signo, posiciones, posicionesMes, setPrecio, porPlataforma, total, invertidoMes, concentracion, dolarDe, sinPrecio, preciosViejos, mesVista, viendoPasado, ym }) {
  ...
  const totalMostrar = viendoPasado ? invertidoMes : total;
                ...
                <li key={p.k} className="pos">
                  <div className="pos-fila">
                    <div className="inv-izq">
                      ...
                      {!viendoPasado && p.rend && (
                        <div className="rend izq">
                          ...
                          <span className={p.rend.real >= 0 ? "ok" : "mal"}>{pctS(p.rend.real)} real</span>
                          <span className={p.rend.usd >= 0 ? "ok" : "mal"}>{pctS(p.rend.usd)} en USD</span>
                        </div>
                      )}
                    ...
                    <div className="pos-der">
                      <span className="monto">{valorCelda ? `${signo}${fmt(conv(valorCelda, viendoPasado ? mesVista : ym))}` : "sin precio"}</span>
                      {!viendoPasado && !p.inst.fijo && (
                      ...
                      <button className="btn-fantasma" onClick={() => setAbierta(abierta === p.k ? null : p.k)}>
                        {abierta === p.k ? "ocultar compras" : `ver ${p.lotes.length === 1 ? "la compra" : "las compras"}`}
                      </button>
                    </div>
                  </div>
                  ...
                  {abierta === p.k && (
                    <ul className="lotes">
                      {p.lotes.slice().sort((a, b) => b.fechaCompra.localeCompare(a.fechaCompra)).map((l) => (
                        <li key={l.id}>
                          <span className="li-fecha">{l.fechaCompra} · {l.plataforma}</span>
        ...
        {lista.length > 0 && (
          <p className="pie">
            {viendoPasado
              ...
              : <>El rendimiento <b>real</b> descuenta la inflación desde el mes de cada compra, por separado. El de <b>USD</b> usa el dólar del día en que compraste, no el de hoy.</>}
          </p>
        )}
      </div>
    </div>
...
function Metas({ d, add, del, ahorroUSD, colchonListo }) {
  ...
  const analizar = (m) => {
    const falta = m.costoUSD - m.llevoUSD;
    if (falta <= 0) return { estado: "listo", texto: "Ya llegaste. Andá." };
    if (!m.fecha) {
      if (ahorroUSD <= 0) return { estado: "mal", texto: "Sin fecha y sin ahorro, no hay forma de estimar." };
      const k = Math.ceil(falta / ahorroUSD), x = new Date(); x.setMonth(x.getMonth() + k);
      return { estado: "ok", texto: `A tu ritmo llegás en ${k} ${k === 1 ? "mes" : "meses"}, en ${mesLargo(x.toISOString().slice(0, 7))}.` };
    }
    const dias = diasHasta(m.fecha);
    if (dias < 0) return { estado: "mal", texto: `La fecha pasó hace ${-dias} días y todavía te faltan US$${fmt(falta)}.` };
    const meses = Math.max(dias / 30.44, 0.1);
    const necesario = falta / meses;
    const base = `Faltan ${dias} ${dias === 1 ? "día" : "días"}. Necesitás guardar US$${fmt(necesario)} por mes.`;
    if (ahorroUSD <= 0) return { estado: "mal", texto: `${base} Hoy no estás ahorrando nada.` };
    if (ahorroUSD >= necesario) return { estado: "ok", texto: `${base} Hoy ahorrás US$${fmt(ahorroUSD)}: vas a llegar.` };
    const k = Math.ceil(falta / ahorroUSD);
    const tarde = Math.max(1, Math.round(k * 30.44 - dias));
    return { estado: "atrasado", texto: `${base} Hoy ahorrás US$${fmt(ahorroUSD)}, así que llegarías unos ${tarde} días tarde.` };
  };
      ...
      {d.metas.map((m) => {
        ...
        return (
          <div className="tarjeta" key={m.id}>
            ...
            <div className="barra"><div style={{ width: `${p}%`, background: a.estado === "atrasado" ? "#CE757B" : a.estado === "mal" ? "#8E2F3E" : "#1E6B55" }} /></div>
            <div className="meta-cifras"><span>US${fmt(m.llevoUSD)} de US${fmt(m.costoUSD)}</span><span className="meta-pct">{fmt(p, 0)}%</span></div>
            <p className={`meta-proy ${a.estado === "mal" ? "mal" : ""}`}>{a.texto}</p>
          </div>
        );
      })}
    </div>
...
function Registro({ d, guardar, conv, signo, registro, valorSaldo, totalLiquido, ultimoMesSaldo, ym }) {
  ...
  const guardarSaldo = () => {
    ...
    if (!p && !pl && !u) { // borrar el saldo del mes
      const s = { ...d.saldos }; delete s[mes]; guardar({ ...d, saldos: s }); return;
    }
...
.app{--papel:#E7EAE3;--alto:#F5F7F2;--tinta:#16232B;--verde:#1E6B55;--rojo:#8E2F3E;--niebla:#8A968F;--linea:#D3D9CF;
background:var(--papel);color:var(--tinta);font-family:Inter,system-ui,sans-serif;min-height:100vh;padding:0 16px 72px;max-width:960px;margin:0 auto;font-size:15px}
...
header.barra{display:flex;align-items:center;justify-content:space-between;padding:20px 0 16px}
.logo{font-family:'Bricolage Grotesque',sans-serif;font-weight:700;font-size:19px;letter-spacing:-.02em}
...
.btn-icono{background:none;border:1px solid var(--linea);border-radius:3px;padding:6px 8px;color:var(--niebla);cursor:pointer;display:flex}
.btn-icono:hover{color:var(--tinta);border-color:var(--tinta)}
.aviso{padding:9px 12px;background:var(--tinta);color:var(--papel);font-size:12.5px;border-radius:3px;margin-bottom:14px}
.offline{padding:10px 13px;background:var(--alto);border:1px dashed var(--linea);border-radius:4px;font-size:12.5px;color:var(--niebla);margin-bottom:14px;line-height:1.55}
...
.hero-numero{font-family:'IBM Plex Mono',monospace;font-size:clamp(38px,8.5vw,58px);font-weight:500;letter-spacing:-.03em;line-height:1.05;margin:6px 0 14px}
...
.tabs{display:flex;gap:2px;margin:22px 0 18px;overflow-x:auto}
.tab{background:none;border:none;border-bottom:2px solid transparent;padding:8px 11px;font-size:13px;color:var(--niebla);cursor:pointer;display:flex;align-items:center;gap:6px;white-space:nowrap;font-family:inherit}
.tab:hover{color:var(--tinta)}
.tab.on{color:var(--tinta);border-bottom-color:var(--tinta);font-weight:500}

.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}
.ancho{grid-column:1/-1}
.tarjeta{background:var(--alto);border:1px solid var(--linea);border-radius:4px;padding:18px}
...
.app input,.app select{background:var(--papel);border:1px solid var(--linea);border-radius:3px;padding:9px 10px;font-size:14px;color:var(--tinta);font-family:'IBM Plex Mono',monospace;width:100%}
.app input:focus,.app select:focus{outline:2px solid var(--verde);outline-offset:-1px}

.btn-primario{background:var(--tinta);color:var(--papel);border:none;border-radius:3px;padding:11px 17px;font-size:13.5px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:7px;font-family:inherit;font-weight:500}
.btn-primario:hover{background:#22333D}
.btn-secundario{background:none;color:var(--tinta);border:1px solid var(--tinta);border-radius:3px;padding:10px 16px;font-size:13.5px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:7px;font-family:inherit}
.btn-fantasma{background:none;border:none;color:var(--niebla);font-size:12px;cursor:pointer;font-family:inherit;text-decoration:underline;text-underline-offset:3px;padding:0}
.btn-fantasma:hover{color:var(--tinta)}
.btn-peligro{background:none;color:var(--rojo);border:1px solid var(--rojo);border-radius:3px;padding:8px 14px;font-size:13px;cursor:pointer;font-family:inherit}
...
@media(max-width:760px){
  ...
  .paso{padding:18px 42px 18px 16px}
  .onb{padding:44px 0}
